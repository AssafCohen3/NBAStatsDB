...
class ResourceDependentDataAbs(ABC, Generic[T]):
    def __init__(self, depends_on_resources: List[Type[ResourceSpecificationAbc]]):
        self._lock = Lock()
        self.data: Optional[T] = None
        self.depends_on_resources: List[Type[ResourceSpecificationAbc]] = depends_on_resources
        self.last_updated: Dict[str, Optional[float]] = {
            resource.get_id(): None
            for resource in self.depends_on_resources
        }
    ...
    def to_refresh(self, session: scoped_session) -> bool:
        ...
        last_updates_stmt = (
            ...
            where(Resource.ResourceId.in_(list(map(lambda r: r.get_id(), self.depends_on_resources))))
        )
