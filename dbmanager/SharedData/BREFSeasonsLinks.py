...
class BREFSeasonsLink(SharedDataResourceAbc):
    ...
    def min_nba_season(self):
        return min(self.get_nba_seasonos(), key=lambda s: s.season).season

    def max_nba_season(self):
        return max(self.get_nba_seasonos(), key=lambda s: s.season).season

    def get_nba_season(self, season: int) -> Optional[BREFSeasonLink]:
        to_ret = [s for s in self.get_nba_seasonos() if s.season == season]
        if len(to_ret) == 0:
            return None
        return to_ret[0]

    def get_nba_seasons_in_range(self, start_season: Optional[int], end_season: Optional[int]) -> List[BREFSeasonLink]:
        return [s for s in self.get_nba_seasonos() if (start_season is None or s.season >= start_season) and (end_season is None or s.season <= end_season)]

    def get_nba_seasons_not_in_list(self, seasons_list: List[int]) -> List[BREFSeasonLink]:
        return [s for s in self.get_nba_seasonos() if s.season not in seasons_list]
